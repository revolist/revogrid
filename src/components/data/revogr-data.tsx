...
import {Pin, VirtualPositionItem} from '../../interfaces';
    ...
    for (let row of this.rows) {
      ...
      for (let col of this.cols) {
        const dataProps = {
          ...
          class: `${CELL_CLASS} ${dataProvider.isReadOnly(row.itemIndex, col.itemIndex, this.pinned) ? DISABLED_CLASS : ''}`,
          style: { width: `${col.size}px`, transform: `translateX(${col.start}px)`}
        };
        cells.push(<div {...dataProps}>{dataProvider.cellRenderer(row.itemIndex, col.itemIndex, this.pinned)}</div>);
      }
