...
import { AutoSizeRowPlugin, type AutoSizeRowConfig } from '../../plugins/row.auto-size.plugin';
...
export class RevoGridComponent {
	...
	/**
	 * Конфигурация автоматического изменения высоты строк.
	 * Включает автоматическое изменение высоты строк в зависимости от контента.
   ...
   */
	...
	@Listen('ondblclickinit') onDblClickInit(
	...
	private setCorePlugins(pluginData: PluginProviders) {
		...
		// Регистрируем плагин автоматического изменения высоты строк
		if (this.autoSizeRow) {
			this.pluginService.add(new AutoSizeRowPlugin(
				this.element,
				pluginData,
				typeof this.autoSizeRow === 'object' ? this.autoSizeRow : undefined
			));
		}
	}
